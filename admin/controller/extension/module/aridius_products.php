	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
				if (!isset($this->request->get['module_id'])) {
					$this->model_setting_module->addModule('aridius_products', $this->request->post);
				} else {
				...
				$this->response->redirect($this->url->link('extension/module/aridius_products', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true));
				} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/aridius_products', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/aridius_products', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/aridius_products', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/aridius_products', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['name_pr'])) {
			$data['name_pr'] = $this->request->post['name_pr'];
		} elseif (isset($module_info['name_pr'])) {
			$data['name_pr'] = $module_info['name_pr'];
		} else {
			$data['name_pr'] = '';
		}
		...
		} elseif (isset($module_info['modules'])) {
			$data['modules'] = $module_info['modules'];
		} else {
			$data['modules'] = '';
		}
		...
		if (isset($this->request->post['colleft'])) {
			$data['colleft'] = $this->request->post['colleft'];
		} elseif (isset($module_info['colleft'])) {
			$data['colleft'] = $module_info['colleft'];
		} else {
			$data['colleft'] = '';
		}
		...
		$this->response->setOutput($this->load->view('extension/module/aridius_products', $data));
	}

	protected function validate() {
		
		if (!$this->user->hasPermission('modify', 'extension/module/aridius_products')) {
