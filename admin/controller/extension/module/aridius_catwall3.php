<?php
class ControllerExtensionModuleAridiusCatWall3 extends Controller {
	
	private $error = array();

	   public function index() {

		$this->load->language('extension/module/aridius_catwall3');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('aridius_catwall3', $this->request->post);
			} else {
			...
			$this->response->redirect($this->url->link('extension/module/aridius_catwall3', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true));
            } else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/aridius_catwall3', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/aridius_catwall3', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/aridius_catwall3', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/aridius_catwall3', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['aridius_catwall3_cat'])) {
		$data['aridius_catwall3_cat'] = $this->request->post['aridius_catwall3_cat'];
		} elseif (!empty($module_info)) {
		$data['aridius_catwall3_cat'] = $module_info['aridius_catwall3_cat'];
		} else {
		$data['aridius_catwall3_cat'] = array();
		}
		...
		$this->response->setOutput($this->load->view('extension/module/aridius_catwall3', $data));
		}

		protected function validate() {

		if (!$this->user->hasPermission('modify', 'extension/module/aridius_catwall3')) {
