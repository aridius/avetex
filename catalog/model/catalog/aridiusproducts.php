<?php
class ModelCatalogAridiusProducts extends Model {
	...
	public function getTotalProductSpecialsCounter() {
		$query = $this->db->query("SELECT COUNT(DISTINCT ps.product_id) AS total FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_end > NOW()) AND (ps.date_end  > ps.date_start))");
	...
	public function getTotalResults() {						
		$query = $this->db->query("SELECT COUNT(DISTINCT r.`product_id`) AS total FROM `" . DB_PREFIX . "review` r, `" . DB_PREFIX . "product` p  WHERE r.`status` = 1 AND p.`product_id` = r.`product_id` AND p.`status` = 1" );
	...
	public function getMostViewed($limit) {
		...
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p INNER JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed DESC LIMIT " . (int)$limit);
	...
	public function getMostRatedProducts($limit) {
		...
		$product_data = $this->cache->get('product.mostrated.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit);
		...
		if (!$product_data) { 
			...
			$this->cache->set('product.mostrated.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'). '.' . $customer_group_id . '.' . (int)$limit, $product_data);
		}
